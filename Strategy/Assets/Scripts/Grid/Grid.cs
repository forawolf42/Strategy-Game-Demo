...
public class Grid : MonoBehaviour {
	...
	void CheckWalkable()
	{
		Vector3 worldBottomLeft = transform.position - Vector3.left * gridWorldSize.x/2 - Vector3.up * gridWorldSize.y/2;
		for (int x = 0; x < gridSizeX; x ++) {
			for (int y = 0; y < gridSizeY; y ++) {
				Vector3 worldPoint = worldBottomLeft + Vector3.left * (x * nodeDiameter + nodeRadius) + Vector3.up * (y * nodeDiameter + nodeRadius);
